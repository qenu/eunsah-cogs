...
class Bnsraid(commands.Cog):
    ...
    def __init__(self, bot: Red) -> None:
        ...
        default_global = {"raids": {}, "emote": "✅", "cancel": "❎"}
    ...
    async def raid(self, ctx: commands.Context, *, content: str):
        """
        創建隊伍報名清單
        ...
        message = await ctx.send(
            embed=discord.Embed.from_dict({"description": "處理中..."})
        )
        ...
        async with self.config.raids() as raids:
            raids[str(message.id)] = {
                ...
                "embed": {
                    ...
                    "fields": [{"name": "參加人員：0", "value": "-"}],
                    "footer": {
                        "text": "id: {}\n召集人: {} ".format(
                            str(message.id), str(ctx.author.display_name)
                        )
                    },
                },
                "signups": {},
    ...
    async def on_raw_reaction_add(
        ...
        if message_id in raid:
            ...
            if payload.emoji.name == await self.config.cancel():
                if str(payload.user_id) == str(author) or self.bot.is_owner(
                    payload.member
                ):
    ...
    async def _embed_updater(self, message_id: str, message=False):
        async with self.config.raids() as raids:
            ...
            raids[message_id]["embed"]["fields"][0]["name"] = "參加人員：" + str(
                len(username)
            )
    ...
    async def _raid_delete(self, message_id: str):
        async with self.config.raids() as raids:
            ...
            raids[message_id]["embed"]["title"] = (
                "(已關閉) " + raids[message_id]["embed"]["title"]
            )
    ...
    async def raid_comment(
        self, ctx: commands.Context, message_id: str, comment: Optional[str]
    ):
        """
        於報名後加上註解，空白可以移除註解
        ...
        async with self.config.raids() as raids:
            try:
                raids[message_id]["signups"][str(ctx.author.id)] = (
                    str(ctx.author.display_name) + " " + comment
                )
            except KeyError:
    ...
    async def bnschar(self, ctx: commands.Context, bns_charname: str):
        ...
        """
        equip_url = (
            "http://g.bns.tw.ncsoft.com/ingame/bs/character/data/equipments?c={}"
        )
        ability_url = (
            "http://g.bns.tw.ncsoft.com/ingame/bs/character/data/abilities.json?c={}"
        )
        ...
        if j["result"] != "success":
            ...
            return
        embed = {"title": bns_charname, "color": ctx.author.color.value, "fields": []}
        embed["fields"].append(
            {
                "name": "攻擊力",
                "value": str(j["records"]["total_ability"]["int_attack_power_value"]),
                "inline": 1,
            }
        )
        embed["fields"].append(
            {
                "name": "血量",
                "value": str(j["records"]["total_ability"]["int_max_hp"]),
                "inline": 1,
            }
        )

        try:
            with requests.get(equip_url.format(bns_charname)) as r:
                soup = bs4(r.text)
        except Exception:
            await ctx.send("伺服器連接中斷")

        weapon = (
            soup.select("div.wrapWeapon")[0]
            .select("div.name")[0]
            .select("span")[0]
            .text
        )
        ring = (
            soup.select("div.accessoryArea")[0]
            .select("div.ring")[0]
            .select("div.name")[0]
            .select("span")[0]
            .text
