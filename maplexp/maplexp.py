...
class Maplexp(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config = Config.get_conf(
            self, identifier=int(str(AUTH_UID) + "001"), force_registration=True
        )
        self.base_time = datetime.timestamp(
            datetime.strptime("1900/01/01", time_string_format)
        )
        ...
        default_user = {"ptr_d": "", "usr_d": {}}
        self.config.register_user(**default_user)

    def _dict_to_embed(
        self, title: str, name: str, data_d: dict, usr_c: discord.User.color
    ) -> discord.Embed:
        """
        ...
        e = discord.Embed(description=title, color=usr_c)
        ...
        if aim:
            ...
            diff = aim - net
            if avg_exp != 0:
                day_req = round(diff / avg_exp + 0.5)
                try:
    ...
    async def _ctx_permissions(self, ctx, admin=True) -> bool:
        ...
        have_perm = (
            int(ctx.author.id) == AUTH_UID or ctx.author.guild_permissions.administrator
            if admin
            else int(ctx.author.id) == AUTH_UID
        )
        if not have_perm:
    ...
    async def _update(
        self, ctx: commands.Context, level: str, exp: str, char: str = None
    ):
        """ """
        if char is None:
        ...
        async with self.config.user(ctx.author).usr_d() as udc:
            ...
            try:
                ...
                if old_date != self.base_time:
                    ...
                    new_avg = round(
                        (net - old_net) / (date_td.total_seconds() / 86400)
        ...
        e.add_field(
            name="經驗成長幅度",
            value=f"{exp_growth:,} ({growth_perc:,.2f}%) {symbol}",
            inline=False,
        )
        if aim:
            val = exp_growth / aim
    ...
    async def _show_info(self, ctx, char: str = None, user: discord.User = None):
        ...
        e = self._dict_to_embed(
            title=str(user.name) + "的玩家資料", name=char, data_d=tar_d, usr_c=user.color
        )
    ...
    async def maple_create(
        ...
        async with self.config.user(user).usr_d() as ud:
            ...
            ud[char]["net_exp"] = net
            ud[char]["date"] = datetime.timestamp(
                datetime.strptime(date, time_string_format)
            )
    ...
    async def maple_list(self, ctx, user: discord.User = None):
        ...
        async with self.config.user(user).usr_d() as ud:
            u_size = len(ud)
            for item in ud:
                date = datetime.fromtimestamp(ud[item]["date"]).strftime(
                    time_string_format
                )
                net = ud[item]["net_exp"]
                level, exp = self._net_levelexp(net)
        ...
        e = discord.Embed(description=user.display_name + "的角色列表", color=user.color)
    ...
    async def maple_set_aim(
        self, ctx, target_level: Optional[int], char: Optional[str]
    ):
        """
        設定目標等級 空白或是0可以移除
    ...
    async def maple_set_reset_avg(
        self, ctx: commands.Context, char: Optional[str], user: Optional[discord.User]
    ):
        """
        重置日平均
    ...
    async def maple_raffle(self, ctx: commands.Context, winner: Optional[int]):
        """
        xp 抽獎系統
        """
        usr_l = []
